...
Core idea: machine unlearning is basically the flip side of continual learning.
CL keeps old knowledge while learning new stuff; MU keeps the rest while
removing specific data. Same tension, opposite direction.

We take A-GEM's gradient projection from the CL literature and flip it:
project the forget gradient onto the half-space where retain loss won't
increase. This gives us a per-step guarantee that the retain set stays safe.

On top of projection, we add:
  - Multi-layer representation erasure (MSE between forget activations 
    and retain-mean activations at intermediate layers)
  - KD anchoring from frozen original model (keeps retain outputs stable)
  - kl_retain: instead of pushing forget outputs to uniform (easy for MIA 
    to detect), push them toward the model's natural confusion on retain set
  - Dual stopping: retain-loss budget + FA target + intra-epoch FA check

NOTE: BN layers MUST be in eval mode during unlearning. If left in train mode,
the running statistics are corrupted because the forget batches consist of only
one class, which causes accuracy to collapse.

TODO: The gradient projection currently acts on the raw gradient, not the
momentum-augmented update. This means the formal retain-safety guarantee
doesn't strictly hold with SGD+momentum. Haven't found a clean fix yet.
"""
...
# ---------- model loading ----------
# pretrained=True loads ImageNet weights, helps a lot on small datasets
...
def _attach_medmnist_targets(ds):
    """MedMNIST uses 'labels' not 'targets' — this shim fixes that so our
    forget/retain splitting code works the same way as for CIFAR/SVHN."""
...
def get_dataset(dataset_name, data_dir='./data'):
    ...
    # cifar100 was tested during development but not included in final results

    # pathmnist needs special handling for labels
    ...
    if dataset_name == 'pathmnist':
        ...
        train_set = PathMNIST(
            ...
            size=28,                           # native res, resize in transform
...
def split_forget_retain(train_set, forget_type, forget_pct=0.01,
    ...
    print(f"  Forget subset: {len(forget_set)} | Retain subset: {len(retain_set)}")
    return forget_set, retain_set


# hooks for grabbing intermediate activations
...
def select_monitor_layers(model):
    # hardcoded for resnet, would need to change for vit/vgg
    candidates = []
    for name, module in model.named_modules():
...
# gradient projection (A-GEM style)

def project_gradient(g_forget, g_retain):
    # project forget grad onto the half-space where retain loss wont increase
    # g_proj = g_f - (dot(g_f,g_r) / ||g_r||^2) * g_r  when dot < 0
    # note: acts on raw gradient not momentum-augmented, so the formal
    # guarantee is weaker with SGD+momentum. budget catches leakage though
    dot = sum((gf * gr).sum() for gf, gr in zip(g_forget, g_retain))

    if dot >= 0:
        return g_forget, False  # objectives are compatible, no conflict

    sq_norm = sum((gr ** 2).sum() for gr in g_retain)
    if sq_norm < 1e-12:
        return g_forget, False  # retain gradient vanishingly small, skip
...
# representation erasure helpers
...
def representation_erasure_loss(forget_acts, retain_acts):
    # push forget activations toward retain batch mean at each hooked layer
    # note: we use per-batch means here which is a bit noisy, so rep_weight
    # needs to stay low. computing global means once would probably help
    total = 0.0
    count = 0
...
def purge_unlearn(model, forget_loader, retain_loader, device, args):
    ...
    # each step: fwd retain -> fwd forget -> compute both grads -> project -> step
    print(f"\n{'=' * 60}")
    ...
    # Pre-compute retain confusion distribution
    # Instead of pushing forget outputs to uniform (which is easy for MIA to catch
    # because real models never produce perfectly uniform outputs), we push them
    # toward what the model naturally outputs on the retain set.
    ...
    # BN must stay in eval mode -- if we let it train, the forget batches
    # (all same class) mess up the running mean/var and acc tanks to ~21%.
    model.train()
    ...
    for epoch in range(args.epochs):
        ...
        if epoch_retain_loss > retain_budget:
            print(f"  WARNING: Retain loss {epoch_retain_loss:.4f} > budget "
                  f"{retain_budget:.4f} -- stopping after this epoch.")
...
def compute_mia(model, forget_loader, forget_class_test_loader, device):
    # MIA: compare forget-train vs forget-class-test using loss + confidence
    # both groups are same class, only difference is train vs test membership
    # AUROC=0.5 means model cant tell them apart (ideal)
    # using AUROC not accuracy to avoid class imbalance issues
...
def compute_feature_distance(model, original_model, loader, device):
    # measure how much the penultimate features changed after unlearning
    # cosine sim near 1.0 = not much changed, lower = more erased
    # mostly driven by class-specific features though
    model.eval()
    original_model.eval()
...
# quick training loop for base model
...
def main():
    ...
    p.add_argument('--dataset', default='cifar10',
                   choices=['cifar10', 'pathmnist', 'mnist', 'svhn', 'stl10'])
    ...
    p.add_argument('--rep_weight', type=float, default=0.1,
                   help='representation erasure weight')
    p.add_argument('--kd_weight', type=float, default=1.0,
                   help='KD anchoring weight')
    ...
    p.add_argument('--forget_gate', type=float, default=0.9,
                   help='entropy gate threshold, 0 to disable')
    p.add_argument('--retain_budget_factor', type=float, default=5.0,
                   help='stop when retain loss > initial * this')
    p.add_argument('--fa_target', type=float, default=None,
                   help='target FA pct, default=random chance')
    p.add_argument('--fa_check_freq', type=int, default=10,
                   help='check FA every N batches (0=epoch-end only)')
    ...
    p.add_argument('--seed', type=int, default=42)  # fixed for reproducibility
    ...
    # load yaml config if provided -- CLI args take priority over config values
    if args.config is not None:
        ...
        # figure out which args the user explicitly typed on the command line
        import sys as _sys
        _argv_keys = set()
        for _tok in _sys.argv[1:]:
            if _tok.startswith('--'):
                _argv_keys.add(_tok.lstrip('-').replace('-', '_').split('=')[0])
        # apply config values only for args NOT explicitly passed on CLI
    ...
    # Build forget-class test loader for correct MIA evaluation
    # (same class, train vs test split — NOT forget-train vs full-test)
    ...
    if args.checkpoint:
        ...
        # Quick 5-epoch training just for demo purposes
        # For real experiments, train properly with the train_base.sh script
    ...
    # ---------- PURGE ----------
    if args.epochs == 0:
        print("\n  epochs=0, skipping unlearning (base model only)")
        history = {'forget_ce': [], 'retain_loss': [], 'rep_loss': [],
                   'proj_rate': [], 'gate_rate': [], 'epoch_fa': []}
        elapsed = 0.0
    else:
    ...
    # TODO: integrate history dict JSON saving for post-hoc analysis
